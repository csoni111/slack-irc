...
class Bot {
  ...
  attachListeners() {
    this.slack.rtm.on('open', () => {
      const bot = this.slack.rtm.dataStore.getUserById(this.slack.rtm.activeUserId);
      this.slackBotId = bot.profile.bot_id;
  ...
  sendToIRC(message) {
    ...
    if (message.subtype === 'bot_message' && message.bot_id === this.slackBotId) {
      return;
    }
    ...
    let username = '';
    if (user) {
      username = this.ircUsernameFormat.replace(/\$username/g, user.name);
      if (this.muteUsers.slack.indexOf(user.name) !== -1) {
        logger.debug(`Muted message from Slack ${user.name}: ${message.text}`);
        return;
      }
    } else {
      username = this.ircUsernameFormat.replace(/\$username/g, message.username);
    }
    ...
    if (ircChannel) {
      ...
      if ('attachments' in message && message.attachments.length > 0 && 'fallback' in message.attachments[0]) {
